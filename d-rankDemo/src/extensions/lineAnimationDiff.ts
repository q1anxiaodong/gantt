...
import type { RankLineSeriesOption as LineSeriesOption } from './RankLineSeries';
...
function diffData(oldData: SeriesData, newData: SeriesData) {
    ...
    newData.diff(oldData)
        ...
        .remove(function (idx) {
            diffResult.push({cmd: '-', idx: idx, idx1: idx});
        })
        .execute();
...
export default function lineAnimationDiff(
    ...
    oldValueOrigin: NonNullable<LineSeriesOption['areaStyle']>['origin'],
    newValueOrigin: NonNullable<LineSeriesOption['areaStyle']>['origin']
) {
