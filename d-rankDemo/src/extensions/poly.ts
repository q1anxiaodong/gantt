...
// import type { ECPolyline } from 'echarts/types/src/chart/line/poly'
...
export class EPolyline extends ECPolyline {
    ...
    getDisplayblePointOn(xOrY: number, dim: 'x' | 'y'): number[] {
        if (!this.path) {
            this.createPathProxy();
            this.buildPath(this.path, this.shape);
        }
        ...
        for (let i = 0; i < data.length;) {
            ...
            switch (cmd) {
                ...
                case CMD.M:
                    ...
                    //     y4 = data[i - 1];
                    // }
                    // t = isDimX ? (xOrY - x4) / (x0 - x4)
                    //     : (xOrY - y4) / (y0 - y4);
                    // if (t <= 1 && t >= 0) {
                    //     const val = isDimX ? (y0 - y4) * t + y4
                    //         : (x0 - x4) * t + x4;
                    //     return isDimX ? [xOrY, val] : [val, xOrY];
                    // }
                    break;  
